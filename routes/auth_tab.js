...
router.get('/tabulation/auth/callback', async function (req, res, next) {
  ...
  try {
    tabulationOauth2.authorizationCode.getToken(tokenConfig).then(async function(result){
      ...
      res.cookie('tabulation_access_token',accessToken['token']['access_token'], { maxAge: 43200000, httpOnly: false });
      res.cookie('tabulation_id_token',accessToken['token']['id_token'], { maxAge: 43200000, httpOnly: false });
      res.cookie('userinfo',JSON.stringify(userInfo), { maxAge: 43200000, httpOnly: false });
